...
def _read_exif_if_image(content: bytes):
    try:
        ...
        if gps_info:
            # on stocke brut, c’est déjà bien pour ton projet
...
def classify_evidence(vt_result, vol_result):
    """
    Retourne (verdict, reason) où verdict ∈ {"threat","suspicious","probably_benign","unknown"}.
    C'est une heuristique très simple pour ton projet.
    """
    score = 0
    reasons = []
    ...
    # ---- Un tout petit exemple avec Volatility ----
    vol = vol_result or {}
    netscan_out = vol.get("netscan_stdout") or ""
    # Si on voit beaucoup de connexions ESTABLISHED, on rajoute un point
    if "ESTABLISHED" in netscan_out:
        score += 1
        reasons.append("ESTABLISHED network connections present in netscan output")

    # ---- Traduction du score en verdict texte ----
    if score >= 4:
        verdict = "threat"
    elif score >= 2:
        verdict = "suspicious"
...
def _hash_gcs_blob_streaming(blob, chunk_size=8 * 1024 * 1024):
    """Calcule (sha256, md5) sans charger tout l'objet en mémoire."""
    sha256 = hashlib.sha256()
    md5 = hashlib.md5()

    import tempfile
    with tempfile.NamedTemporaryFile() as tmp:
        # Téléchargement sur le disque (/tmp), pas en RAM
...
def compute_verdict(vt_result, vol):
    """
    Retourne un petit dict {label, score, reasons[]} basé sur VT + Volatility.
    """
    score = 0
    reasons = []
    ...
    # 2) malfind : régions mémoire exécutable / RWX
    ...
    # 3) cmdline : outils connus / lignes suspectes
...
def dfir_ingest(cloud_event):
    ...
    # 👉 D’ABORD on récupère le blob
    bucket = storage_client.bucket(bucket_name)
    blob = bucket.blob(name)
    blob.reload()  # pour avoir size, content_type, etc.
    ...
    if _is_memory_dump(name, mime_type):
        ...
        if size_bytes > BIG_DUMP_THRESHOLD:
            # Gros dump (ex: 256 Mo) → on ne le lit pas du tout
            logging.info(
                "Huge memory dump (%d bytes) for %s – skipping hashing & VT",
                size_bytes,
                name,
            )
            # sha256/md5 restent None → pas de VT
        else:
            logging.info(
    ...
    # --- VirusTotal : seulement si on a un hash ---
    vt_result = None
    if sha256 or md5:
        vt_result = vt_lookup(sha256 or md5) or vt_lookup(md5 or sha256)

    # --- Volatility uniquement sur les dumps ---
    volatility = None
    if _is_memory_dump(name, mime_type):
